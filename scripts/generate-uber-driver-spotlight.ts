// Bespoke redesign of the Uber (Drive/Deliver) spotlight Reel — same
// treatment as generate-mercury-spotlight.ts / generate-coinbase-spotlight.ts:
// real Uber brand colors (from Brandfetch: accent #276EF1, true black bg)
// and the actual Uber wordmark animated in, instead of the generic
// sky-photo template / emerald-amber palette every other spotlight reel
// uses. Uber's identity has no separate icon glyph — the wordmark IS the
// mark — so that's what gets the bouncy entrance here instead of a symbol.
//
// Usage: node scripts/generate-uber-driver-spotlight.ts
...
// Uber's real brand marks (fetched via Brandfetch, uber.com), not the
// generic EMERALD/AMBER used on every other spotlight reel.
const ACCENT = "#276EF1"; // Uber's primary accent blue
const ACCENT_LIGHT = "#a9c2f7"; // lighter tint for secondary copy on the dark bg
const DARK_BG = "#161616"; // near-black, close to Uber's own pure #000000 but
// with a touch of grey so it doesn't read as pure black on a phone screen
const DARK_BG_2 = "#000000"; // Uber's own dark, for the background gradient
    ...
    ...rings.map((r, i) =>
      h("div", {
        ...
        style: {
          ...
          height: r.size,
          borderRadius: "50%",
          backgroundColor: `rgba(39,110,241,${r.alpha})`,
        },
      })
    )
  );
...
async function run() {
  const offer = offers.find((o) => o.slug === "uber-driver");
  if (!offer) {
    console.error('No offer with slug "uber-driver" in src/data/offers.ts');
    process.exit(1);
  }

  const FRAMES_DIR = "scripts/output/spotlight-frames-uber-driver";
  const OUT_FILE = "public/social/spotlight-uber-driver.mp4";
  const HERO_TEXT = "$1,750";

  await rm(FRAMES_DIR, { recursive: true, force: true });
  await mkdir(FRAMES_DIR, { recursive: true });
  await mkdir("public/social", { recursive: true });

  // Uber's wordmark is a wide 800x267 crop (3:1) rather than a square
  // symbol — sized to roughly match the visual footprint of the square
  // logomark treatments used for Mercury/Coinbase.
  const LOGO_WIDTH = 480;
  const LOGO_HEIGHT = Math.round((LOGO_WIDTH * 267) / 800);

  const [logoBuffer, qrCodeDataUrl] = await Promise.all([
    readFile("public/uber-logo.png"),
  ...
  function renderFrame(time: number) {
    ...
    const logoEntranceRotate = (1 - logoP) * -8; // subtler rotate for a wide wordmark
    const floatY = Math.sin(time * 1.8) * 8 * logoP;
    const floatRot = Math.sin(time * 1.2 + 1) * 1.5 * logoP;
    ...
    return h(
      ...
      h(
        ...
        h(
          ...
          `to drive with ${offer.name.split(" (")[0]}`
        ),
        h(
          "div",
          {
