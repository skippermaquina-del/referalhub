// Bespoke redesign of the Coinbase spotlight Reel — same treatment as
// generate-mercury-spotlight.ts: real Coinbase brand colors (from
// Brandfetch: accent #0052FF, dark #0A0B0D) and the actual Coinbase
// logomark animated in, instead of the generic sky-photo template /
// emerald-amber palette every other spotlight reel uses.
//
// Usage: node scripts/generate-coinbase-spotlight.ts
...
// Coinbase's real brand marks (fetched via Brandfetch, coinbase.com), not
// the generic EMERALD/AMBER used on every other spotlight reel.
const ACCENT = "#0052FF"; // Coinbase's primary accent blue
const ACCENT_LIGHT = "#8fb0ff"; // lighter tint for secondary copy on the dark bg
const DARK_BG = "#14161c"; // near-black, close to Coinbase's own #0A0B0D but with
// a touch of blue so it doesn't read as pure black on a phone screen
const DARK_BG_2 = "#0A0B0D"; // Coinbase's own dark, for the background gradient
    ...
    ...rings.map((r, i) =>
      h("div", {
        ...
        style: {
          ...
          height: r.size,
          borderRadius: "50%",
          backgroundColor: `rgba(0,82,255,${r.alpha})`,
        },
      })
    )
  );
...
async function run() {
  const offer = offers.find((o) => o.slug === "coinbase");
  if (!offer) {
    console.error('No offer with slug "coinbase" in src/data/offers.ts');
    process.exit(1);
  }

  const FRAMES_DIR = "scripts/output/spotlight-frames-coinbase";
  const OUT_FILE = "public/social/spotlight-coinbase.mp4";
  const HERO_TEXT = "$10 in crypto";
  ...
  const [logoBuffer, qrCodeDataUrl] = await Promise.all([
    readFile("public/coinbase-symbol.png"),
